...
(function($) {
	...
	$("#color-switcher-content").on('click','.js__change_color',function(event){
		...
		if (!$(this).hasClass('active')){
			...
			if ($(this).data('color') != 'blue'){
				$("body").append('<link id="custom-color-themes" rel="stylesheet" href="template/styles/color/' + $(this).data('color') + '.min.css">');
			}
	...
	$("#color-reset").on('click',function(){
		...
		if (selector.data('color') != 'blue'){
			$("body").append('<link id="custom-color-themes" rel="stylesheet" href="template/styles/color/' + selector.data('color') + '.min.css">');
		}
